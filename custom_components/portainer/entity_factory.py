...
class EntityFactory:
    ...
    def _validate_entity(self, temp_obj, description, uid=None):
        try:
            ...
            if uid is not None:
                _LOGGER.error("Error accessing properties of entity %s (uid: %s): %s", description.key, uid, e)
            else:
                _LOGGER.error("Error accessing properties of entity %s: %s", description.key, e)
            return None, None
    ...
    def _is_valid_entity(self, unique_id, entity_name, description, uid=None):
        if not unique_id or unique_id.strip() == "":
            if uid is not None:
                _LOGGER.warning("Skipping entity creation for %s (uid: %s): unique_id is None or empty (%s)", description.key, uid, repr(unique_id))
            else:
                _LOGGER.warning("Skipping entity creation for %s: unique_id is None or empty (%s)", description.key, repr(unique_id))
            return False
        if not entity_name or entity_name.strip() == "":
            if uid is not None:
                _LOGGER.warning("Skipping entity creation for %s (uid: %s): name is None or empty (%s)", description.key, uid, repr(entity_name))
            else:
                _LOGGER.warning("Skipping entity creation for %s: name is None or empty (%s)", description.key, repr(entity_name))
            return False
        return True
    ...
    def _final_entity_validation(self, entity):
        try:
            ...
            if not unique_id or unique_id.strip() == "":
                _LOGGER.error("Filtering out entity with invalid unique_id: %s (name: %s, entity_id: %s)", repr(unique_id), repr(entity_name), repr(entity_id))
                return False
            if not entity_name or entity_name.strip() == "":
                _LOGGER.error("Filtering out entity with invalid name: %s (unique_id: %s, entity_id: %s)", repr(entity_name), repr(unique_id), repr(entity_id))
                return False
            _LOGGER.debug("Final entity validation passed: unique_id=%s, name=%s, entity_id=%s", unique_id, entity_name, entity_id)
            return True
        except (AttributeError, TypeError, KeyError) as e:
    ...
    def _add_entity_if_valid(self, new_entities, temp_obj, description, uid=None):
        ...
        if any(e.unique_id == unique_id for e in new_entities):
            _LOGGER.debug("Entity with unique_id %s already in new_entities, skipping", unique_id)
            return
        if uid is not None:
            _LOGGER.debug("Adding entity with uid to new_entities: unique_id=%s, name=%s, uid=%s, type=%s", unique_id, entity_name, uid, type(temp_obj).__name__)
        else:
            _LOGGER.debug("Adding entity to new_entities: unique_id=%s, name=%s, type=%s", unique_id, entity_name, type(temp_obj).__name__)
